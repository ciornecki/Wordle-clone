...
let currentCol = 0 /* current letter */
	...
	document.body.onkeydown = e => {
		...
		if (key == 'Enter') {
			if (currentCol == boardCols) {
				let duration = 500
				let word = boardArr[currentRow].reduce((prev, curr) => prev + curr)
				if (validWord(word)) {
					revalWord()
					currentRow++
					currentCol = 0
...
function revalWord() {
	...
	for (let i = 0; i < boardCols; i++) {
		...
		tile.style.animationDelay = `${
			(i * animationDuration) / 2
		}ms` /*  divided by 2 so its * 250 and its exactly in the middle of animation (at 50%) and allows for smooth color change*/
